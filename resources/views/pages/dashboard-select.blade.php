            ...
            <div class="">
                <div class="d-flex">                        
                    ...
                    @elseif($lastCampaign && $lastCampaign->status == 'inactive')
                        ...
                        <p class="mb-0 ml-4" id='campaign-name' style="font-weight: bold; font-size: 20px; text-transform: uppercase;"></p>
                    </div>
                    @else
                        <button type="submit" class="btn btn-primary " style=" margin-right:20px"  data-toggle="modal" data-target="#modal" >Create Campaign</button>
                        <div class="dropdown">
                    ...
                    <div class="col-xl-5 col-lg-12 col-md-6 col-sm-12 col-12 layout-spacing">
                        <div class="widget-content widget-content-area br-6">
                            ...
                            <div class="pt-3 d-flex justify-content-center">
                                {{ $devices->links('pagination::simple-bootstrap-4') }}
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @push('script')
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                var campaignId =  {{ request('id') }}; // Pastikan menggunakan variabel yang benar

                console.log('Campaign ID:', campaignId); // Cek nilai kampanyeId di konsol

                $.ajax({
                    type: 'GET',
                    url: '/campaign/name/' + campaignId,
                    success: function(response) {
                        var campaignName = response.campaign[0].name;
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                function updateChartData(chart, label, data) {
                    ...
                    // Tambahkan label dan data yang baru
                    chart.data.labels.push(label);
                    chart.data.datasets.forEach((dataset) => {
                        dataset.data.push(data);
                    });
                    ...
                    $('#tbl_list').DataTable({
                        ...
                        columns: [{
                            ...
                            },
                            {
                                ...
                                name: 'id',
                                render: function(data, type, row) {
                                    var inspectRoute = "{{ route('wifi-inspect', ['id' => ':id']) }}";
                                    return '<a type="button" href="' + inspectRoute.replace(':id', data) + '"   class="btn btn-primary"><i class="ik ik-edit-2"></i>Inspect</a>&nbsp;<a style="display:none;" type="button" class="btn btn-warning"><i class="ik ik-crosshair"></i>Attack</a>';
                                }
                            }
                        ]
                    });
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.status == "OK") {
                                ...
                                // console.log("Data Api", data.data.clients.length);
