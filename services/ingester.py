...
def toDBApp(data):
	...
	#time.sleep(1)
	if result is None:
		queryInsert = "INSERT INTO devices (`bssid`,`ssid`,`signal`,`channel`,`crypto`,`created_at`,`updated_at`,`type`,`attackmode`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
		insertVal = (data['bssid'], data['ssid'], data['power'], data['channel'], data['enc'], datetime.now(),datetime.now(), "Ap", attackmode)
...
def toDBClient(data):
	...
	if result is None:
		#print("Client empty")
		queryInsert = "INSERT INTO clients (`bssid`,`appbssid`,`signal`,`created_at`,`updated_at`) VALUES (%s,%s,%s,%s,%s)"
		insertVal = (data['bssid'], data['appbssid'], data['power'], datetime.now(),datetime.now())
		mysqlCursor.execute(queryInsert, insertVal)
		mysqlDb.commit()
...
def ingest():
	try:
		...
		print("Ingester error, continue")
		pass
if __name__ == '__main__':
	while True:
		ingest()
	os.system("rm "+stringFile)
